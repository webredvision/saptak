...
import TableThree from "@/app/(admin)/admin/Tables/TableThree"
  ...
  return (
    <>
      ...
      <div className="w-full max-w-full rounded-[10px] dark:shadow-card">
        <div className="bg-[var(--rv-bg-white)] dark:bg-[var(--rv-bg-gray)]-dark px-5 py-4 rounded mb-5">
            ...
            <div className="flex gap-x-2">
              {packageData.map((item, index) => (
                <div className="mx-1" key={index}>
                  <Button
                    className="text-[var(--rv-white)] px-3 py-1 bg-primary hover:bg-[var(--rv-bg-purple)] rounded"
        ...
        <div className="rounded-[10px] border border-stroke bg-[var(--rv-bg-white)] p-4 shadow-1 dark:border-dark-3 dark:bg-[var(--rv-bg-gray)]-dark dark:shadow-card sm:p-7.5">
          <div className="max-w-full grid lg:grid-cols-5 md:grid-cols-3 grid-cols-5 gap-5">
            {allAmcsLogos.filter(logo => logo.logocategory == logoCategory)
              ...
              allAmcsLogos
                ...
                .map((item, index) => (
                  <div
                    className={`rounded-[10px] border-2 ${item.status ? "border-[var(--rv-green)]" : "border-[var(--rv-red)]"
                      } bg-[var(--rv-bg-white)] p-2 shadow-1 dark:bg-[var(--rv-bg-gray)]-dark dark:shadow-card sm:p-4 text-center flex flex-col items-center`}
                    key={index}
                  >
                    <div className="flex justify-end gap-3 mb-3">
                      <button
                        className={`flex justify-center rounded-[7px] px-3 py-[7px] font-medium ${item.status
                          ? "bg-[var(--rv-bg-green)] text-[var(--rv-white)]"
                          : "bg-[var(--rv-bg-red)] text-[var(--rv-black)]"
                          }`}
                        type="button"
                        onClick={() =>
                      ...
                      <button
                        className="flex justify-center rounded-[7px] border border-stroke px-3 py-[7px] font-medium text-dark hover:shadow-1 dark:border-dark-3 dark:text-[var(--rv-white)] hover:dark:text-[var(--rv-red)]"
                        type="submit"
                      ...
                      <button
                        className="flex justify-center rounded-[7px] border border-[var(--rv-gray-dark)] px-3 py-[7px] font-medium text-[var(--rv-gray)]-2 hover:bg-opacity-90"
                        type="submit"
