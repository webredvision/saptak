...
export async function DELETE(req, { params }) {
    ...
    try {
        ...
        const category = await GalleryCategoryModel.findByIdAndDelete(id);
...
export async function GET(req, { params }) {
    ...
    try {
        await ConnectDB(); // Ensure DB connection
        const blog = await GalleryCategoryModel.findById(id); // Properly await the findById function
...
export async function PUT(req, { params }) {
    ...
    try {
        await ConnectDB();
        const updatedBlog = await GalleryCategoryModel.findByIdAndUpdate(
            id,
            { title: categorytitle },
        );
