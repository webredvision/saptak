...
export default function BlogDetailsPage2({ blog, blogs }) {
  ...
  return (
    <div className="bg-[var(--rv-bg-black)] text-[var(--rv-white)]">
      ...
      <div className="px-4">
        <div className="max-w-7xl mx-auto main-section flex flex-col md:flex-row items-start gap-5 w-full">
          <div className="md:w-2/3 w-full flex flex-col gap-5 md:gap-8">
            <Link
              href="/blogs"
              className="inline-flex items-center text-[var(--rv-primary)] hover:text-[var(--rv-primary)] transition-colors"
            >
          ...
          <div className="md:w-1/3 w-full flex flex-col gap-5 md:gap-8 sticky top-20">
            <Heading align="start" title={"Recent Blogs"} />

            <div className="w-full flex flex-col gap-5">

              {relatedBlogs.length === 0 ? (
                <p className="text-sm text-[var(--rv-gray)]">No recent blogs in this category.</p>
              ) : (
                relatedBlogs.map((article, index) => (
                  <div
                    key={index}
                    className="group bg-[var(--rv-bg-secondary)] md:h-32 grid grid-cols-1 md:grid-cols-3 rounded-xl overflow-hidden transition-all duration-300"
                  >
                    ...
                    <div className="p-4 flex flex-col justify-between gap-3 col-span-2">
                      <p className="line-clamp-2 font-bold leading-snug group-hover:text-[var(--rv-primary-dark)] transition-colors">
                        {article?.metatitle}
                      </p>
