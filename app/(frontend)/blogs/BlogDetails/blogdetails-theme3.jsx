...
export default function BlogDetailsPage3({ blog, blogs }) {
  ...
  return (
    <div className="bg-[var(--rv-bg-white)] text-[var(--rv-black)]">
      ...
      <div className="px-4">
        <div className="max-w-7xl mx-auto main-section flex flex-col lg:flex-row items-start gap-5 w-full">
          <div className="md:w-2/3 w-full flex flex-col gap-5 md:gap-8">
            <Link
              href="/blogs"
              className="inline-flex items-center hover:text-[var(--rv-primary)] transition-colors"
            >
            ...
            <div className="w-full flex flex-col gap-5">
              ...
              {relatedBlogs.length === 0 ? (
                ...
                relatedBlogs.map((article, index) => (
                  <div
                    key={index}
                    className="group border backdrop-blur-xl md:h-32 grid grid-cols-1 md:grid-cols-3 rounded-xl overflow-hidden transition-all duration-300"
                  >
                    ...
                    <div className="p-4 flex flex-col justify-between gap-3 col-span-2">
                      <p className="line-clamp-2 font-bold leading-snug transition-colors">
                        {article?.metatitle}
                      </p>

                      <p className="flex-grow line-clamp-2 md:hidden">
                        {article?.description}
                      </p>
