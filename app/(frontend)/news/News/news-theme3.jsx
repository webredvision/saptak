...
export default function NewsTheme3({ newsData }) {
    ...
    <div className="relative overflow-hidden text-[var(--rv-black)] bg-[var(--rv-bg-white)]">
      ...
      <div className="px-4 relative z-10">
        <section className="max-w-7xl mx-auto main-section flex flex-col gap-6 md:gap-8">
          ...
          {categories.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 w-full overflow-hidden bg-[var(--rv-bg-primary-light)] p-1 rounded-full max-w-2xl mx-auto">
              {categories.map((tab) => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`px-4 py-3 rounded-full font-medium transition-all duration-300 ${activeTab === tab
                    ? "bg-[var(--rv-bg-primary)] text-[var(--rv-white)] shadow-md"
                    : "bg-[var(--rv-bg-white)] text-[var(--rv-black)] hover:bg-[var(--rv-bg-primary)] hover:text-[var(--rv-white)]"
                    }`}
                >
                  {tab === "ipo"
                    ? "IPO News"
