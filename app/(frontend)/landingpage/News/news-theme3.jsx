"use client";
import React, { useState, useEffect } from "react";
import { motion } from "framer-motion";
import Link from "next/link";
import { FaArrowRight } from "react-icons/fa";
import { BiCalendar } from "react-icons/bi";
import Button from "@/app/components/Button/Button";
import Heading from "@/app/components/Heading/Heading";

export default function NewsTheme3({ newsData }) {
    const [loading, setLoading] = useState(true);
    const [activeTab, setActiveTab] = useState("ipo");

    useEffect(() => {
        const timer = setTimeout(() => setLoading(false), 1000);
        return () => clearTimeout(timer);
    }, []);

    const categories = Object.keys(newsData || {});
    const activeData = newsData?.[activeTab] || [];

    const container = {
        hidden: {},
        show: { transition: { staggerChildren: 0.2 } },
    };

    return (
        <div className="px-4 bg-[var(--rv-bg-white)]">
            <section className="max-w-7xl mx-auto main-section flex flex-col gap-6 md:gap-8">
                <div className="flex items-center justify-between gap-5 w-full flex-wrap">
                    <Heading align="start" description={"Latest News & Updates"} heading={"News"} />
                    {!loading && (
                        <div className="">
                            <Button
                                link={"/news"}
                                text={"All News"}
                            />
                        </div>
                    )}
                </div>

                <div className="flex items-center gap-5 flex-wrap">
                    {categories.map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`font-medium transition-all text-[var(--rv-primary)] py-1 duration-300 ${activeTab === tab
                                ? "border-b-2 border-[var(--rv-primary)]"
                                : ""
                                }`}
                        >
                            {tab === "ipo"
                                ? "IPO News"
                                : tab === "market"
                                    ? "Market News"
                                    : "Popular News"}
                        </button>
                    ))}
                </div>

                <motion.div
                    variants={container}
                    initial="hidden"
                    whileInView="show"
                    viewport={{ once: true, amount: 0.2 }}
                    className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5"
                >
                    {loading
                        ? Array.from({ length: 3 }).map((_, i) => (
                            <div
                                key={i}
                                className="bg-[var(--rv-bg-gray-light)] animate-pulse rounded-xl overflow-hidden flex flex-col"
                            >
                                <div className="w-full h-48 bg-[var(--rv-bg-gray)]"></div>
                                <div className="p-5 flex flex-col gap-3">
                                    <div className="w-24 h-4 bg-[var(--rv-bg-gray)] rounded"></div>
                                    <div className="w-3/4 h-5 bg-[var(--rv-bg-gray)] rounded"></div>
                                    <div className="w-full h-4 bg-[var(--rv-bg-gray)] rounded"></div>
                                    <div className="w-1/2 h-4 bg-[var(--rv-bg-gray)] rounded"></div>
                                </div>
                            </div>
                        )) : !Array.isArray(activeData) ? (
                            <p className="text-center text-[var(--rv-red-dark)] font-medium col-span-3">
                                {newsData?.[activeTab]?.error || "No data available"}
                            </p>
                        ) : (
                            activeData?.slice(0, 3)?.map((news, i) => (
                                <motion.div
                                    key={news._id || news.id || i}
                                    initial={{ opacity: 0, y: 40 }}
                                    whileInView={{ opacity: 1, y: 0 }}
                                    transition={{ duration: 0.5, delay: i * 0.1 }}
                                    className="bg-[var(--rv-bg-white)] overflow-hidden"
                                >
                                    <Link
                                        href={news?.link} target="_blank"
                                    >
                                        <img
                                            src={news?.img || news?.image || "/no-image.jpg"}
                                            alt={news?.title}
                                            className="w-full h-80 object-cover rounded-xl"
                                        />
                                        <div className="p-5 flex flex-col gap-3">

                                            <h6 className="font-semibold text-[var(--rv-primary)] line-clamp-2">
                                                {news?.title}
                                            </h6>
                                            <p className="line-clamp-2">
                                                {news?.desc || news?.description}
                                            </p>
                                        </div>
                                    </Link>
                                </motion.div>
                            ))
                        )}
                </motion.div>
            </section>
        </div >
    );
}
